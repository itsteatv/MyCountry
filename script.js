...
const renderCountry = function(data, className = "") {
    const html = `
        ...
        <p class="country__row"><span>👫</span>Population: ${(+data.population / 1000000).toFixed()}M</p>
        <p class="country__row"><span>🗣️</span>Language: ${data.languages[0].name}</p>
...
const getJSOn = function(url, err = "Something went wrong") {
    ...
    // Checking response
    if(!response.ok)
    throw new Error(`${err} (${response.status} error)`);
...
const getCountryData = function(country) {
    ...
    .catch(err => {
        ...
        renderError(`Something went wrong ⚠️⚠️⚠️ ${err.message} Please Try again!`)
        }
    ).finally(() => {
        countriesContainer.style.opacity = 1;
    });
